...
class Diffusion(BaseModule):
    ...
    @torch.no_grad()
    def reverse_diffusion_raw(self, z, mask, mu, n_timesteps, stoc=False, spk=None):
        h = 1.0 / n_timesteps
        xt = z * mask
        for i in range(n_timesteps):
            ...
            time = t.unsqueeze(-1).unsqueeze(-1)
            noise_t = get_noise(time, self.beta_min,
                                self.beta_max, cumulative=False)
    ...
    @torch.no_grad()
    def reverse_diffusion_dpm(self, z, mask, mu, n_timesteps, stoc, spk=None):
        ns = self.solver
        xt = z * mask
        yt = xt - mu
        T = 1
        ...
        for i in range(n_timesteps):
            ...
            lt = 1 - torch.exp(-get_noise(s, self.beta_min, self.beta_max, cumulative=True))
    ...
    def forward(self, z, mask, mu, n_timesteps, stoc=False, spk=None):
        return self.reverse_diffusion_dpm(z, mask, mu, n_timesteps, stoc, spk)
